...
module doorClip(frameless=false, servoMount=false) {
    ...
    translate([strokeSize + 2 * clipWall - (leaverSwitchLength - clipSize) - 2 * thinWall - leaverExtension, clipSize / 2 + clipWall + 3 * boltM + 0 * thinWall /* - leaverSwitchThickness - 0 * gapToFit*/, 0]) {
        union() {
            difference() {
                union() {
                    cube([2 * thinWall + leaverSwitchLength, 2 * thinWall + leaverSwitchThickness + 2 * gapToFit, leaverSwitchContactLength + leaverSwitchWidth]);
                    if (frameless) {
                        translate([0,0,-clipWall]) {
                            cube([2 * thinWall + leaverSwitchLength, 2 * thinWall + leaverSwitchThickness + (2 + 1) * gapToFit, clipWall + 0.01]);   
                        }
                    }
                }
                ...
                #translate([thinWall + leaverSwitchContactOffset, leaverSwitchThickness + 1 * gapToFit + thinWall - 0.01, -0.01]) {
                    cube([leaverSwitchLength - 2 * leaverSwitchContactOffset, thinWall + gapToFit + 0.02, leaverSwitchContactLength + 0.02]);
                }
            }
    ...
    if (servoMount) {
        difference() {
            ...
            translate([strokeSize + 2 * clipWall + 2 * thinWall + 4,  -clipSize / 2 - clipWall - 3 * boltM - 0.01, -0.01]) {
                //sphere(r=2);
                cube([clipSize - 2 * thinWall, 3 * boltM, strokeSize - 2 * thinWall]);
                if (frameless) {
                    translate([0,0,-clipWall]) {
    ...
    translate([0,0,servoWidth / 2]) {
        if (servoMount) {
            translate([strokeSize - strokeSize * cos(angleForT($t)), 0, 0]) {
                servoLink(length=strokeSize - servoArmM/2 - servoSpindleM/2);
            }
        } else {
...
module frameClip(frameless=false) {
    translate([strokeSize + clipSize + doorClipGap + 2 * clipWall + gapToMove, -clipSize / 2 - gapToMove - 3 * boltM,0]) {
        difference() {
            ...
            doorLatchPannelSize = gapToMove * 2 + clipSize;
            translate([-0.01, 3 * boltM, -0.01]) {
                cube([doorLatchPannel + 0.02, doorLatchPannelSize , clipWall + 2 * gapToMove]);
            }

            translate([doorLatchRounding - 0.01, 3 * boltM + doorLatchPannelSize, -doorLatchRounding ]) {
                rotate([0,-90,90]) {
                    cornerRoundCut(doorLatchRounding, gapToMove * 2 + clipSize);
                }
            }

            translate([doorLatchRounding - 0.01, 3 * boltM + doorLatchPannelSize, clipWall + doorLatchRounding ]) {
                rotate([0, 90, 0]) {
                    rotate([90, 90, 0]) {
                        cornerRoundCut(doorLatchRounding, gapToMove * 2 + clipSize);
                    }
            ...
            translate([doorLatchRounding - 0.01, 3 * boltM + doorLatchPannelSize + doorLatchRounding -0.01, -doorLatchRounding]) {
                rotate([0, 0, 0]) {
                    rotate([0, 0, 180]) {
                        cornerRoundCut(doorLatchRounding, clipWall + 2 * doorLatchRounding);
                    }
                }
            }

            translate([doorLatchRounding - 0.01, 3 * boltM - doorLatchRounding, -doorLatchRounding]) {
                rotate([0, 0, 0]) {
                    rotate([0, 0, 90]) {
                        cornerRoundCut(doorLatchRounding, clipWall + 2 * doorLatchRounding);
                    }
    ...
    translate([strokeSize/2 + clipSize + 2 * clipWall, clipSize / 2 + clipWall + 3 * boltM + 0 * thinWall - ( 0 * leaverSwitchThickness), leaverSwitchContactLength + leaverSwitchWidth + 0.8 * leaverThickness]) {
        difference() {
            union() {
                translate([gapToMove, 0, 0]) {
                    cube([ strokeSize / 2 + doorClipGap, 2 * thinWall + leaverSwitchThickness, 4 * thinWall]);
                }
            }
    ...
    translate([strokeSize + clipSize + doorClipGap + 2 * clipWall + gapToMove, clipSize / 2 + clipWall + 1 * boltM /* + 2 * thinWall */ - (0 * leaverSwitchThickness), 0]) {
        difference() {
            union() {
    ...
    translate([strokeSize/2 + clipSize + 2 * clipWall, 0, leaverSwitchContactLength + leaverSwitchWidth + 0.8 * leaverThickness]) {
        difference() {
            union() {
                translate([gapToMove, -(2 * boltM+(2 * gapToMove)+thinWall)/2, -leaverSwitchContactLength]) {
                    cube([ strokeSize / 2 + doorClipGap, 2 * boltM+(2 * gapToMove)+thinWall, 4 * thinWall + leaverSwitchContactLength]);
                }
...
module laserDoorSide(frameless=false, servoMount=false, showWood=true, showPerspex=true, showSVG=true) {
    ...
    acsPannelOffsetToEdge=originOffsetToEdge + (servoMount ? 0 : servoMountPlateLength / 2 + servoMountPlateOffset) + 6 * boltM + acsButtonM / 2;
            ...
            difference() {
                union() {
                    ...
                    if (showPerspex) {
                        translate([originOffsetToEdge - doorWoodBorder, 0, -doorWoodThickness - 0.01]) {
                            translate([perspexBorderSize, 0, doorWoodThickness]) {
                                color("grey",0.5) mirror([1,0,0]) windowWithPeninsula(
                                    [boxSize[0] - doorWoodHingeBorder - doorWoodBorder + 2 * perspexBorderSize, boxSize[2] - 2 * doorWoodBorder + 2 * perspexBorderSize],
                                    [acsControlPeninsulaSize - doorWoodBorder, acsLidWidth - 2 * perspexBorderSize],
                                    perspexThickness,
                                    perspexBorderSize
                                );
                            }
                ...
                translate([0, 0,(frameless ? -doorWoodThickness : -doorWoodThickness) - 0.01]) {
                    linear_extrude(height=(frameless ? doorWoodThickness : doorWoodThickness) + 0.02) {
                        translate([(servoMount ? 0 : -(servoMountPlateLength / 2) - servoMountPlateOffset) - 6 * boltM, 0, 0]) {
                            rotate([0,0,180]) {
                ...
                translate([originOffsetToEdge - doorWoodBorder, 0, -doorWoodThickness - 0.01]) {
                    mirror([1,0,0]) windowWithPeninsulaHoles([boxSize[0] - doorWoodHingeBorder - doorWoodBorder, boxSize[2] - 2 * doorWoodBorder], [acsControlPeninsulaSize - doorWoodBorder, acsLidWidth], perspexBorderSize, boltM, doorWoodThickness + perspexThickness + 0.02);
                }
            }
        }
    }
